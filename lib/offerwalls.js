    ...
    const offerwalls = [
      {
        ...
        reward: 0.5 * getLevelMultiplier(user.level), // Reward increases with level
      ...
      },
      {
        ...
        reward: 0.3 * (1 + (getLevelMultiplier(user.level) - 1) * 0.5),
      ...
      },
      {
        ...
        reward: 5.0 * (1 + (getLevelMultiplier(user.level) - 1) * 5),
      ...
      },
      {
        ...
        reward: 1.0 * getLevelMultiplier(user.level),
...
export const completeOfferwall = async (userId, offerwallId) => {
  try {
    ...
    switch(offerwallId) {
      case 1:
        reward = 0.5 * getLevelMultiplier(user.level);
        break;
      case 2:
        reward = 0.3 * (1 + (getLevelMultiplier(user.level) - 1) * 0.5);
        break;
...
export const getOfferwallStats = async (userId) => {
  try {
    ...
    const stats = {
      todayEarnings: 1.5 * getLevelMultiplier(user.level),
