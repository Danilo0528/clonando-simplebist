...
import { debitBound, creditTokens } from './economy.js'; // Importar helpers
...
export const requestWithdrawal = async (userId, amount, walletAddress, cryptoCurrency = 'BTC') => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const updatedUser = await tx.user.findUnique({ where: { id: parseInt(userId) } });
...
export const cancelWithdrawal = async (userId, withdrawalId) => {
  try {
    ...
    import prisma from './prisma.mjs';
    import { debitBound, creditBound } from './economy.js'; // Importar creditBound
