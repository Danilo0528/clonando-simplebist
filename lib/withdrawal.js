...
import { debitBound, creditBound } from './economy.js';
...
export const requestWithdrawal = async (userId, amount, walletAddress, cryptoCurrency = 'BTC') => {
  try {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: {
        id: true,
        boundTokenBalance: true // Usar boundTokenBalance
      }
    });
    ...
    // Check if user has sufficient balance (Bound)
    if (user.boundTokenBalance < amount) {
      throw new Error('Insufficient bound token balance for withdrawal');
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      // 1. Update user's bound balance
      await debitBound(userId, amount, tx);
    ...
    return {
      ...
      newUserBoundBalance: result.updatedUser.boundTokenBalance,
...
export const getWithdrawalStats = async (userId) => {
  try {
    ...
    const stats = {
      availableBalance: user.boundTokenBalance,
      minWithdrawal: WITHDRAWAL_CONFIG.MIN_WITHDRAWAL_AMOUNT,
      maxWithdrawal: Math.min(user.boundTokenBalance, WITHDRAWAL_CONFIG.MAX_WITHDRAWAL_PER_DAY),
...
export const cancelWithdrawal = async (userId, withdrawalId) => {
  try {
    ...
    const withdrawal = await prisma.withdrawal.findFirst({
        where: { id: parseInt(withdrawalId), userId: userId }
    });
    ...
    await prisma.$transaction(async (tx) => {
        // Retornar balance a bound
        await creditBound(userId, withdrawal.amount, tx);
